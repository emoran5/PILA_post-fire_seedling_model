#G model
#G7 - Intercept, Site, Year/age, Ht, ElevXSite, Clim

load("PILASdl_Data.RData")
...
####### Setup design matrix
#X1 <- cbind(rep(1,N.grow),X.Sites.G,X.Yr.G,X.Ht.G,X.Elev.G,X.Sno.G) 
#X1 <- cbind(rep(1,N.grow),X.Sites.G,X.Yr.G,X.Ht.G,X.Elev.G,X.P.G)  
#X1 <- cbind(rep(1,N.grow),X.Sites.G,X.Yr.G,X.Ht.G,X.Elev.G,X.JMax.G) 
X1 <- cbind(rep(1,N.grow),X.Sites.G,X.Yr.G,X.Ht.G,X.Elev.G,X.JMin.G) 
#X1 <- cbind(rep(1,N.grow),X.Sites.G,X.Yr.G,X.Ht.G,X.Elev.G,X.CWD.G) 


I <- ncol(X1)  #columns of design matrix

###### Priors
Bpm <- c(5,rep(0,3),rep(0,4),0.15,rep(0,8),0)
#Intercept = 5 cm annual growth; 0 site-level and year means; 
Bps <- c(6,rep(6,3),rep(6,4),6,rep(6,8),6)   #variances
...
###Starting values
Beta <- c(runif(1,2,10),runif(3,-2,2),runif(4,-2,2),runif(1,-0.1,0.5),runif(8,-2,2),runif(1,-1,1))
...
rownames(outpar) <- c('Int','site17','site18','site19','Yr1','Yr2','Yr3','Yr4','Ht','30','40','45','50','52','55','60','75','Clim','sig')
outpar
